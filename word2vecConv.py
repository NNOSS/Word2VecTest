...
parser.add_argument(
    '--log_dir',
    type=str,
    default=os.path.join(current_path, 'conv_log'),
...
def generate_batch(batch_size, num_back):
  global data_index
  #assert batch_size % num_skips == 0
  #assert num_skips <= 2 * skip_window
  batch = np.ndarray(shape=(batch_size, num_back), dtype=np.int32)
  ...
  #data_index += span
  #print("data_index %i" %data_index)
  for i in range(batch_size):
    #context_words = [w for w in range(span) if w != skip_window]
    #words_to_use = random.sample(context_words, num_skips)
    context_words = data[data_index - num_back:data_index]
    for j in range(len(context_words)):
      batch[i, j] = context_words[j]
...
with tf.Session() as sess:    
    ...
    with tf.name_scope('conv_ins'):
      conv_train_inputs = tf.placeholder(tf.int32, shape=[batch_size, conv_lookback], name = "train")
      conv_train_labels = tf.placeholder(tf.int32, shape=[batch_size, 1], name = "labels")
      #valid_dataset = tf.constant(valid_examples, dtype=tf.int32, name = "example")

    embed_train = tf.nn.embedding_lookup(embeddings, tf.reshape(conv_train_inputs,[-1]))
    ...
    similarity = tf.matmul(
      embedding_guess, normalized_embeddings, transpose_b=True)
    ...
    for step in xrange(num_steps):
      batch_inputs, batch_labels = generate_batch(batch_size, conv_lookback)
      ...
      if step % step_test == 0:
        _, loss_val, loss_s, sim = sess.run(
                [optimizer, loss, loss_summary, similarity],
                feed_dict=feed_dict)
        for i in xrange(min(batch_size,10)):
          nearest = (-sim[i, :]).argsort()[0]
          close_word = reverse_dictionary[nearest]
          actual_word = reverse_dictionary[batch_labels[i,0]]

          #leadline = batch_inputs[i];

          print("(actual vs guess): %s | %s" %(actual_word, close_word))
      else:
        _, loss_val, loss_s = sess.run(
...
with graph.as_default():
  ...
  with tf.name_scope('inputs'):
    train_inputs = tf.placeholder(tf.int32, shape=[batch_size, conv_lookback])
  ...
  with tf.device('/cpu:0'):
    ...
    with tf.name_scope('embeddings'):
      ...
      embed = tf.nn.embedding_lookup(embeddings, tf.reshape(train_inputs, [-1]))
      embedpic = tf.reshape(embed, [batch_size, conv_lookback, embedding_size])
    ...
    with tf.name_scope('optimizer'):
      #tf.train.AdamOptimizer(lr_v, beta1=beta1).
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    print(batch_inputs)
    feed_dict = {train_inputs: np.expand_dims(batch_inputs, axis=1), train_labels: batch_labels}
    ...
    if step % 10000 == 0:
      # Compute semantically closest words to each word, takes a long time.
      sim = similarity.eval()
